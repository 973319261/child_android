...
public class SideBar extends View {
	...
	private String[] sections = new String[] { "#", "A", "B", "C", "D", "E",
...
//		header.setText(headerString);
		String[] adapterSections = null;
		if (typeEnum == TypeEnum.AreaCode) {
			adapterSections = (String[]) ((CountryAreaCodeAdapter) mListView.getAdapter()).getSections();
		} else if (typeEnum == TypeEnum.Contacts) {
			adapterSections = (String[]) ((ContactAdapter) mListView.getAdapter()).getSections();
		}
		try {
			for (int i = adapterSections.length - 1; i > -1; i--) {
				if (adapterSections[i].equals(headerString)) {
					if (typeEnum == TypeEnum.AreaCode) {
						mListView.setSelection(((CountryAreaCodeAdapter) mListView.getAdapter()).getPositionForSection(i));
					} else if (typeEnum == TypeEnum.Contacts) {
						mListView.setSelection(((ContactAdapter) mListView.getAdapter()).getPositionForSection(i));
					}
					break;
				}
			}
